#!/usr/bin/env python3
"""The Junzi Harness gate: every attributed quotation must trace to a source.
...
which is the exact failure 信 Xìn (honesty) never trades. This gate
...
A repo whose third discipline is 信 Xìn — "invent nothing: no fabricated
number, citation, or source" — cannot ship a citation nobody can test.
"""
...
def check_urls_online(sources: list[Source], findings: list[Finding]) -> None:
    ...
    for s in sources:
        ...
        req = urllib.request.Request(url, method="HEAD", headers={
            "User-Agent": "junzi-harness-citation-gate/1.0"})
        try:
